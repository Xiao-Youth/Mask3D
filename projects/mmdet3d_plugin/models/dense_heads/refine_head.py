...
from mmdet.models.builder import build_neck,build_head,build_loss
from mmcv.cnn import (ConvModule, bias_init_with_prob, normal_init,xavier_init,build_activation_layer,
                      build_norm_layer)
@HEADS.register_module()
class UnifiedRefineHead(DETRHead):
    """Head of Detr3D.
    Args:
    ...
    def __init__(self,
        ...
        super(UnifiedRefineHead, self).__init__(
        ...
        if self.with_stuff:
            self.stuff_query_embedding = nn.Embedding(num_stuff_classes*num_cams,
                                                self.embed_dims*2)
            self.stuff_reference_points = nn.Linear(self.embed_dims, 3)
            self.stuff_bbox=nn.Sequential(Linear(self.embed_dims,self.embed_dims),nn.ReLU(),Linear(self.embed_dims,self.code_size))
        self._init_layers()
    ...
    def forward(self, mlvl_feats, seg_feats,img_metas,queries=None,references=None,query_pos=None,bboxes=None):
        ...
        if self.with_stuff:
            ...
            stuff_query_embed=stuff_query_embed[None].expand(bs,-1,-1)
            stuff_query_pos, stuff_query = torch.split(stuff_query_embed, self.embed_dims, dim=2)
            stuff_bbox=self.stuff_bbox(stuff_query+stuff_query_pos)
            stuff_reference_points=self.stuff_reference_points(stuff_query_pos)
            stuff_bbox[...,[0,1,4]]=stuff_reference_points
            stuff_reference_points=stuff_reference_points.sigmoid()
            queries=torch.cat([queries,stuff_query],dim=1)
            references=torch.cat([references,stuff_reference_points],dim=1)
            bboxes=torch.cat([bboxes,stuff_bbox],dim=1)
        if self.with_query_pos:
            query_pos=torch.cat([query_pos,stuff_query_pos],dim=1)
        else:
            query_pos=None

        hs, init_reference, inter_references,inter_masks,inter_bboxes = self.transformer(
            mlvl_feats[1:5] if len(mlvl_feats)==5 else mlvl_feats,
            seg_feats=seg_feats,
            ...
            query_pos=query_pos,
            bboxes=bboxes,
            refine=True if queries is not None else False,
            seg_branch=self.forward_seg
        ) #hs(nb_dec, bs, num_query, embed_dims)
        hs = hs.permute(0, 2, 1, 3)
        outputs_masks_stuff=None
        if self.with_stuff:
            inter_bboxes=inter_bboxes[:,:,:self.num_query]
            outputs_masks_stuff=inter_masks[:,:,:,self.num_query:]
            inter_masks=inter_masks[:,:,:,:self.num_query]
            hs=hs[:,:,:self.num_query]
            init_reference=init_reference[:,:self.num_query]
            inter_references=inter_references[:,:,:self.num_query]
        outputs_classes = []
        outputs_coords = []
        queries2img_masks=[self.get_queries2img_mask(inter_references[i],img_metas) for i in range(len(inter_references))]
        ...
        for lvl in range(hs.shape[0]):
            ...
            # # _=self.get_queries2img_mask(reference,img_metas)
        ...
        # outputs_masks=self.forward_seg(seg_feats,hs,queries2img_masks)
        outputs_masks=inter_masks
        outputs_classes = torch.stack(outputs_classes)
        outputs_coords = inter_bboxes
        outs = {
    ...
    def _get_target_single(self,
        ...
        gt_masks_for_cost=torch.zeros([6,num_gts,H,W],device=device)
        mask_weights=torch.zeros([6,num_gts],device=device)
        ...
        assign_result = self.assigner.assign(bbox_pred, cls_score,mask_pred, gt_bboxes,
                                             gt_labels,gt_masks_for_cost,mask_weights,gt_mask_labels,gt_assigned_masks,queries2img_masks,gt_bboxes_ignore)
        ...
        mask_targets_weights=torch.zeros([6,num_bboxes],device=device)
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list,
         bbox_weights_list, pos_inds_list, neg_inds_list,mask_targets_list,mask_targets_weights_list) = multi_apply(
            ...
            gt_labels_list, gt_bboxes_list,gt_masks_list,gt_mask_labels_list,gt_assigned_masks_list,queries2img_masks_list, gt_bboxes_ignore_list)
    ...
    def loss_single(self,
        ...
        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list,mask_preds_list,
                                           gt_bboxes_list, gt_labels_list,gt_masks_list,gt_mask_labels_list,gt_assigned_masks_list,queries2img_masks_list,
                                           gt_bboxes_ignore_list)
        ...
        mask_preds=mask_preds.reshape([mask_preds.shape[1],mask_preds.shape[0]*mask_preds.shape[2],mask_preds.shape[3],mask_preds.shape[4]])
        ...
        return loss_cls, loss_bbox,loss_ce,loss_dice
    ...
    def loss(self,
        ...
        losses_cls, losses_bbox,losses_ce,losses_dice= multi_apply(
            self.loss_single, all_cls_scores, all_bbox_preds,all_mask_preds,all_mask_preds_stuff,
            all_gt_bboxes_list, all_gt_labels_list,all_gt_masks_list,all_gt_mask_labels_list,all_gt_assigned_masks,all_gt_masks_stuff_list,all_gt_mask_labels_stuff_list,queries2img_masks,
            all_gt_bboxes_ignore_list)
        ...
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i,loss_ce_i,loss_dice_i in zip(losses_cls[:-1],
                                           losses_bbox[:-1],losses_ce[:-1],losses_dice[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls_refine'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_bbox_refine'] = loss_bbox_i
            loss_dict[f'd{num_dec_layer}.loss_mask_ce_refine'] = loss_ce_i
            loss_dict[f'd{num_dec_layer}.loss_mask_dice_refine'] = loss_dice_i
            num_dec_layer += 1
        return loss_dict
    ...
    def get_seg_results(self,preds_dict,img_metas):
        ...
        cls_scores=preds_dict['all_cls_scores'][-1]
        queries2img_masks=preds_dict['queries2img_masks'][-1]
        # bboxes=self.get_bboxes(preds_dict,img_metas)
        # corners=torch.stack([boxes[0].corners for boxes in bboxes])
        # queries2img_masks=queries2img_masks+self.get_bboxes2img_mask(corners,img_metas)
        B=len(masks)
        num_cam=len(masks[0])
        # H,W=masks.shape[-2:]
        H,W=img_metas[0]['ori_shape'][:2]
        results=[]
        for bi in range(B):
            result=[]
            for vi in range(num_cam):
                ...
                if self.with_stuff:\
                    ...
                    for ci in self.vis_stuff_classes_orders:
                        stuff_mask=preds_dict['all_mask_preds_stuff'][-1][bi,vi,(ci-self.num_classes)*num_cam+vi].sigmoid()
                ...
                for k in sorted_inds:
                    ...
                    if mask_area>0 and original_area>0:
                        ...
                        panoptic_seg[mask]=labels_per_img[k]+1000*current_segment_id
    ...
    def get_bboxes2img_mask(self,corners,img_metas):
        mask=None
        pc_range=self.pc_range
        corners[...,0:1]=(corners[...,0:1]-pc_range[0])/(pc_range[3] - pc_range[0])
        corners[...,1:2]=(corners[...,1:2]-pc_range[1])/(pc_range[4] - pc_range[1])
        corners[...,2:3]=(corners[...,2:3]-pc_range[2])/(pc_range[5] - pc_range[2])
        for i in range(corners.shape[2]):
            if mask is not None:
                mask=mask+self.get_queries2img_mask(corners[:,:,i],img_metas)
            else:
                mask=self.get_queries2img_mask(corners[:,:,i],img_metas)
